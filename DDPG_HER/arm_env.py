...
class ArmEnv(object):
    ...
    def __init__(self, mode='hard', sparse= False):
        # node1 (l, d_rad, x, y),
        # node2 (l, d_rad, x, y)
        self.mode = mode
        self.sparse = sparse
    ...
    def _get_state(self):
        if not self.sparse:
            ...
            # return the distance (dx, dy) between arm finger point with blue point
            arm_end = self.arm_info[:, 2:4]
            t_arms = np.ravel(arm_end - self.center_coord)
            distance = np.ravel(arm_end - self.point_info)
            center_dis = (self.point_info - self.center_coord) / 200
            in_point = 1 if self.grab_counter > 0 else 0
            return np.hstack([in_point, t_arms / 200, center_dis,
                              # arm1_distance_p, arm1_distance_b,
                              ]), distance[-2:]
...
if __name__ == '__main__':
    ...
    for ep in range(20):
        ...
        while True:
            env.render()
            s, r, done, g = env.step(env.sample_action())
